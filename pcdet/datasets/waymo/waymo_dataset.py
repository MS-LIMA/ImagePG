...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        #if self.training:
        #    self.env = lmdb.open('/root/Desktop/workspace/MyFiles/LCANet_v2/waymo_images.lmdb', readonly=True, lock=False, readahead=False)
    ...
    def __getitem__(self, index):
        ...
        if "images" in get_item_list:
            ...
            for cam_key in CAM_KEYS:
                # if self.training:
                #     img_path = os.path.join(sequence_name, str(sample_idx), '{}.jpg'.format(cam_key))
                #     with self.env.begin(write=False) as txn:
                #         img_bytes = txn.get(img_path.encode())
                #         image = Image.open(io.BytesIO(img_bytes)).convert('RGB')
                #         image = np.asarray(image).astype(np.float32) / 255.
                #         # image = self.to_numpy(image)
                #         images.append(image)

                #     # img_path = os.path.join(self.data_path, sequence_name, str(sample_idx), '{}.jpg'.format(cam_key))
        ...
        if 'annos' in info:
            ...
            if gt_boxes_lidar.ndim <= 1:
                gt_boxes_lidar = gt_boxes_lidar[np.newaxis, :]
            ...
            # if self.dataset_cfg.get('FILTER_EMPTY_BOXES_FOR_TRAIN', False):
            ...
            calib = calibration_waymo.Calibration(info['image'])
            input_dict.update({
                'gt_names': annos['name'],
                'gt_boxes': gt_boxes_lidar,
                'num_points_in_gt': annos.get('num_points_in_gt', None),
                'gt_dense': annos['obj_ids'],
                'calib': calib
            })
    ...
    def print_ap_dict(self, ap_dict, comment = ""):
        ap_result_str = '\n'
        for key in ap_dict:
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        def waymo_eval(eval_det_annos, eval_gt_annos):
            ...
            ap_dict = eval.waymo_evaluation(
                copy.deepcopy(eval_det_annos), copy.deepcopy(eval_gt_annos), class_name=class_names,
            ...
            self.print_ap_dict(eval.waymo_evaluation_clamp(
                copy.deepcopy(eval_det_annos), copy.deepcopy(eval_gt_annos), class_name=class_names,
                dist_clamp=[0, 30], fake_gt_infos=self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False)
            ), comment="[0, 30]----------------------------")

            self.print_ap_dict(eval.waymo_evaluation_clamp(
                copy.deepcopy(eval_det_annos), copy.deepcopy(eval_gt_annos), class_name=class_names,
                dist_clamp=[30, 50], fake_gt_infos=self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False)
            ), comment="[30, 50]----------------------------")

            self.print_ap_dict(eval.waymo_evaluation_clamp(
                copy.deepcopy(eval_det_annos), copy.deepcopy(eval_gt_annos), class_name=class_names,
                dist_clamp=[50, 999999], fake_gt_infos=self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False)
            ), comment="[50, inf]----------------------------")
    ...
    def create_gt_database_of_single_scene(self, info_with_idx, database_save_path=None, use_sequence_data=False, used_classes=None,
        ...
        if use_sequence_data and crop_gt_with_tail and False:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_parallel', action='store_true', default=True, help='')
