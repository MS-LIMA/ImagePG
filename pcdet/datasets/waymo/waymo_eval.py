...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def mask_by_distance_clamp(self, dist_clamp, boxes_3d, *args):
        dist = np.linalg.norm(boxes_3d[:, 0:2], axis=1)
        mask = (dist >= dist_clamp[0]) & (dist < dist_clamp[1])
        boxes_3d = boxes_3d[mask]
        ret_ans = [boxes_3d]
        for arg in args:
            ret_ans.append(arg[mask])
    ...
    def waymo_evaluation_clamp(self, prediction_infos, gt_infos, class_name, dist_clamp, fake_gt_infos=True):
        print('Start the waymo evaluation...')
        assert len(prediction_infos) == len(gt_infos), '%d vs %d' % (prediction_infos.__len__(), gt_infos.__len__())
        ...
        pd_boxes3d, pd_frameid, pd_type, pd_score, pd_overlap_nlz = self.mask_by_distance_clamp(
            dist_clamp, pd_boxes3d, pd_frameid, pd_type, pd_score, pd_overlap_nlz
        )
        gt_boxes3d, gt_frameid, gt_type, gt_score, gt_difficulty = self.mask_by_distance_clamp(
            dist_clamp, gt_boxes3d, gt_frameid, gt_type, gt_score, gt_difficulty
        )
...
def main():
    ...
    waymo_AP = eval.waymo_evaluation(
        copy.deepcopy(pred_infos), copy.deepcopy(gt_infos_dst), class_name=args.class_names, distance_thresh=1000, fake_gt_infos=False
    )
    
    waymo_AP_dist1 = eval.waymo_evaluation_clamp(
        copy.deepcopy(pred_infos), copy.deepcopy(gt_infos_dst), class_name=args.class_names, dist_clamp=[0, 30], fake_gt_infos=False
    )

    waymo_AP_dist2 = eval.waymo_evaluation_clamp(
        copy.deepcopy(pred_infos), copy.deepcopy(gt_infos_dst), class_name=args.class_names, dist_clamp=[30, 50], fake_gt_infos=False
    )

    waymo_AP_dist3 = eval.waymo_evaluation_clamp(
        copy.deepcopy(pred_infos), copy.deepcopy(gt_infos_dst), class_name=args.class_names, dist_clamp=[50, 9999], fake_gt_infos=False
    )

    print("OVERALL===============================")
    print(waymo_AP)
    print("[0, 30)===============================")
    print(waymo_AP_dist1)
    print("[30, 50)===============================")
    print(waymo_AP_dist2)
    print("[50, inf)===============================")
    print(waymo_AP_dist3)
