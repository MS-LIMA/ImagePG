...
class WaymoDataset(DatasetTemplate):
            ...
            with open(info_path, 'rb') as f:
                ...
                for info in infos:
                    ...
                    for idx in range(boxes.shape[0]):
                        boxes[idx][2] += boxes[idx][5] * 0.5
                    info['annos']['gt_boxes_lidar'] = boxes

                    if info['annos']['bbox_3d_lidar'].shape[0] <= 0:
                        info['annos']['bbox_3d_lidar'] = np.empty((0, 7), dtype=np.float32)
                    info['annos']['gt_boxes_lidar'] = info['annos']['bbox_3d_lidar']
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        db_info_save_path = save_path / ('%s_waymo_dbinfos_dense_%s_sampled_%d.pkl' % (processed_data_tag, split, sampled_interval))
