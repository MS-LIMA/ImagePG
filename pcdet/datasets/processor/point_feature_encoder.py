...
class PointFeatureEncoder(object):
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        additional_feature_num = self.point_encoding_config.get("ADDITIONAL_FEATS_NUM", 0)
        if additional_feature_num > 0:
            point_feature_list.append(np.zeros((points.shape[0], additional_feature_num)))
        
        point_features = np.concatenate(point_feature_list, axis=1)

        return point_features, True

    def absolute_coordinates_index_encoding(self, points=None):
        if points is None:
            num_output_features = len(self.used_feature_list)
            return num_output_features
        ...
        for x in self.used_feature_list:
            if x in ['x', 'y', 'z']:
                ...
                point_feature_list.append(indices.reshape(-1,1))
        point_features = np.concatenate(point_feature_list, axis=1)
        return point_features, True
