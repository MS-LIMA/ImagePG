...
CAM_KEYS = ['CAM_FRONT', 'CAM_FRONT_LEFT', 'CAM_FRONT_RIGHT', 'CAM_SIDE_LEFT', 'CAM_SIDE_RIGHT']
...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.img_overlap_ratio = sampler_cfg.get('OVERLAP_RATIO', 1.0)
    ...
    def collect_image_crops_kitti(self, info, data_dict, obj_points, sampled_gt_boxes, sampled_gt_boxes2d, idx):
        ...
        valid = data_dict['images'].shape == raw_image.shape

        return new_box, img_crop2d, obj_points, obj_idx, valid
    ...
    def collect_image_crops(self, img_aug_gt_dict, info, data_dict, obj_points, sampled_gt_boxes, sampled_gt_boxes2d, idx):
        if self.img_aug_type == 'kitti':
            new_box, img_crop2d, obj_points, obj_idx, valid = self.collect_image_crops_kitti(info, data_dict,
    ...
    def copy_paste_to_image(self, img_aug_gt_dict, data_dict, points):
        if self.img_aug_type == 'kitti':
            ...
            data_dict = self.copy_paste_to_image_kitti(data_dict, img_aug_gt_dict['gt_crops2d'], img_aug_gt_dict['gt_number'], point_idxes, alpha=self.img_overlap_ratio)
    ...
    def copy_paste_to_image_waymo(self, data_dict, total_valid_sampled_dict, gt_boxes):
        
        calib = data_dict['calib']
        org_gt_num = gt_boxes.shape[0] - len(total_valid_sampled_dict)
        
        image_sizes = [[x.shape[0], x.shape[1]] for x in data_dict['images']]
        gt_boxes_corners = box_utils.boxes_to_corners_3d(gt_boxes)
        
        for image_idx in range(len(image_sizes)):
            gt_boxes_2d_proj, _ = common_utils.corners_to_img_bbox(calib, torch.tensor(gt_boxes_corners), image_sizes[image_idx], 1.0, image_idx)
            gt_boxes_sizes = common_utils.bbox_area_clamped_to_image(gt_boxes_2d_proj.numpy(), image_sizes[image_idx]).astype(np.int32)
            
            image_crops = []
            for gt_idx in range((gt_boxes.shape[0])):
                bbox = gt_boxes_2d_proj[gt_idx].long()
                bbox_size = gt_boxes_sizes[gt_idx] 
                
                if (bbox[2] - bbox[0] <= 1 or bbox[3] - bbox[1] <= 1).item():
                    image_crops.append(None)
                    continue
                
                if gt_idx < org_gt_num:
                    image_crop = data_dict['images'][image_idx][bbox[1]:bbox[3], bbox[0]:bbox[2]]
                    image_crops.append(image_crop)
                else:
                    cam_key = CAM_KEYS[image_idx]
                    sequence_name = total_valid_sampled_dict[gt_idx - org_gt_num]['sequence_name']
                    sample_idx = total_valid_sampled_dict[gt_idx - org_gt_num]['sample_idx']
                    img_path = os.path.join(self.root_path, 'waymo_processed_data_v0_5_0', sequence_name, str(sample_idx), '{}.jpg'.format(cam_key))
                    
                    if os.path.exists(img_path):
                        image = Image.open(img_path)
                        image = np.array(image).astype(np.float32) / 255.

                        image_crop = image[bbox[1]:bbox[3], bbox[0]:bbox[2]] # H, W
                        image_crops.append(image_crop)
                    else:
                        image_crops.append(None)
                
            coords = gt_boxes[:, :3]  # (x, y, z)
            distances = np.linalg.norm(coords, axis=1)  # Euclidean distance
            sorted_indices = np.argsort(-distances)  # descending order
            
            total_image = data_dict['images'][image_idx]         
            for sort_idx in sorted_indices:
                if isinstance(image_crops[sort_idx], np.ndarray):
                    bbox = gt_boxes_2d_proj[sort_idx].long()
                    image_crop = image_crops[sort_idx]
                    total_image[bbox[1]:bbox[3], bbox[0]:bbox[2]] = image_crop
            data_dict['images'][image_idx] = total_image
            
        return data_dict
        
    def copy_paste_to_image_kitti(self, data_dict, crop_feat, gt_number, point_idxes=None, alpha=1.0):
        ...
        for _order in paste_order:
            ...
            image[_box2d[1]:_box2d[3],_box2d[0]:_box2d[2]] = (1-alpha) * image[_box2d[1]:_box2d[3],_box2d[0]:_box2d[2]] + alpha * crop_feat[_order]
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.img_aug_type is not None and img_aug_gt_dict is not None:
        ...
        if self.img_aug_type is not None:
            if self.img_aug_type == 'kitti':
                ...
                data_dict = self.copy_paste_to_image_waymo(data_dict, total_valid_sampled_dict, gt_boxes)
        return data_dict
    ...
    def __call__(self, data_dict):
        ...
        if self.sampler_cfg.get('DISABLE_AFTER', False):
            disable_after = self.sampler_cfg.get('DISABLE_AFTER')
            if hasattr(self, 'epoch'):
                if disable_after >= getattr(self, 'epoch'):
                    sample_group_epochs = self.sampler_cfg.get('SAMPLE_GROUPS_EPOCH', None)
                    if sample_group_epochs and str(getattr(self, 'epoch', 0)) in sample_group_epochs:
                        for x in sample_group_epochs[str(self.epoch)]:
                            class_name, sample_num = x.split(':')
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if self.img_aug_type is not None and self.img_aug_type != 'waymo':
