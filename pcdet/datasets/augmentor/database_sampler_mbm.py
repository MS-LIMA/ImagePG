...
from pcdet.ops import roiaware_pool3d
from pcdet.ops.roiaware_pool3d import roiaware_pool3d_utils

from ...ops.iou3d_nms import iou3d_nms_utils
from ...utils import box_utils, box2d_utils, common_utils, calibration_kitti
...
class DataBaseSamplerMBM(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.use_oais = sampler_cfg.get("USE_OAIS", False)
        self.use_raw = sampler_cfg.get("USE_RAW", True)    
        self.use_mask = sampler_cfg.get('USE_MASK', True)
    ...
    def copy_paste_to_image_kitti(self, data_dict, crop_feat, gt_number, point_idxes=None):
        kitti_img_aug_type = 'order' # order depth
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        if 'gt_dense' in data_dict:
            gt_dense = data_dict['gt_dense'][gt_boxes_mask]
...
#           obj_points = remove_points_via_distance(obj_points, info)      ## <- FIX HERE?
        ...
        if 'gt_dense' in data_dict:    
            sampled_gt_dense = np.array([x['obj_ids'] for x in total_valid_sampled_dict])
            gt_dense = np.concatenate([gt_dense, sampled_gt_dense], axis=0)
            data_dict['gt_dense'] = gt_dense
        ...
        if self.sampler_cfg.get("UPSAMPLE_POINTS_PATH") is not None:
            
            mm_points_sampled = self.load_upsampled_points(aug_boxes_image_idx, aug_boxes_gt_idx, gt_names, gt_boxes)
            mm_points = data_dict['mm_points']
            
            mm_points = box_utils.remove_points_in_boxes3d(mm_points, large_sampled_gt_boxes) 
            mm_points = np.concatenate([mm_points, mm_points_sampled], axis=0)
    ...
    def add_no_sampled_boxes(self, data_dict, mv_height=None, sampled_gt_boxes2d=None):
        gt_boxes_mask = data_dict['gt_boxes_mask']
        gt_boxes = data_dict['gt_boxes'][gt_boxes_mask]
        gt_names = data_dict['gt_names'][gt_boxes_mask]
        ...
        data_dict['aug_boxes_image_idx'] = gt_idx
        data_dict['aug_boxes_gt_idx'] = boxes_gt_idx
        data_dict['gt_names'] = gt_names
        data_dict['gt_boxes'] = gt_boxes
        
        if self.sampler_cfg.get("UPSAMPLE_POINTS_PATH") is not None:
            mm_points_sampled = self.load_upsampled_points(gt_idx, boxes_gt_idx, gt_names, gt_boxes)
            mm_points = data_dict['mm_points']
            mm_points = np.concatenate([mm_points, mm_points_sampled], axis=0)
            data_dict['mm_points'] = mm_points
    ...
    def load_upsampled_points(self, image_indices, gt_indices, gt_names, gt_boxes):
        ...
        ones_col = np.ones((upsample_points_list.shape[0], 1))  # arr.shape[0]는 N 값
        upsample_points_list = np.concatenate((upsample_points_list, ones_col), axis=1)
        
        return upsample_points_list
    ...
    def __call__(self, data_dict):
        ...
        existed_boxes3d = box_utils.boxes3d_lidar_to_kitti_camera(existed_boxes, data_dict['calib'])
        
        if self.img_aug_type is not None:
            existed_boxes2d = box_utils.boxes3d_kitti_camera_to_imageboxes(existed_boxes3d, data_dict['calib'], data_dict['images'].shape[:2])
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if self.img_aug_type is not None:
                    ...
                    if self.use_oais:
                        ...
                        sampled_boxes2d = torch.Tensor(sampled_boxes2d)
                        existed_boxes2d = torch.Tensor(existed_boxes2d)

                        existed_depth, sampled_depth = existed_boxes[:, 0], sampled_boxes[:, 0]; existed_depth = torch.from_numpy(existed_depth); sampled_depth = torch.from_numpy(sampled_depth)
                        iou2d1 = box2d_utils.pairwise_oais(sampled_boxes2d, existed_boxes2d, sampled_depth, existed_depth).cpu().numpy()
                        iou2d2 = box2d_utils.pairwise_oais(sampled_boxes2d, sampled_boxes2d, sampled_depth, sampled_depth).cpu().numpy()

                        iou2d2[range(sampled_boxes2d.shape[0]), range(sampled_boxes2d.shape[0])] = 0
                        iou2d1 = iou2d1 if iou2d1.shape[1] > 0 else iou2d2
                        valid_mask = ((iou2d1.max(axis=1) < self.img_aug_iou_thresh) & (iou2d2.max(axis=1) < self.img_aug_iou_thresh) & ((iou1.max(axis=1) + iou2.max(axis=1)) == 0))
                ...
                # for idx, x in enumerate(valid_mask):
                #     sample = sampled_dict[idx]
                #     img_path = self.root_path /  f'training/image_2/{sample["image_idx"]}.png'
                #     raw_image = io.imread(img_path)
                #     valid_mask[idx] = valid_mask[idx] and (data_dict['images'].shape == raw_image.shape)
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            data_dict = self.add_no_sampled_boxes(data_dict)
