...
from ...utils import common_utils, box_np_ops, box_utils
import numba
import warnings
from numba.core.errors import NumbaPerformanceWarning
warnings.filterwarnings('ignore', category=NumbaPerformanceWarning)

def random_flip_along_x(gt_boxes, points, bm_points, mm_points=None):
    """
    Args:
    ...
    if enable:
        ...
        points[:, 1] = -points[:, 1]
        bm_points[:, 1] = -bm_points[:, 1]

        if gt_boxes.shape[1] > 7:
            gt_boxes[:, 8] = -gt_boxes[:, 8]
        
        if isinstance(mm_points, np.ndarray):
            mm_points[:,  1] = -mm_points[:, 1]
        
    return gt_boxes, points, bm_points, mm_points, enable

def random_flip_along_y(gt_boxes, points, bm_points, mm_points=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, points, bm_points, mm_points, rot_range):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    bm_points = common_utils.rotate_points_along_z(bm_points[np.newaxis, :, :], np.array([noise_rotation]))[0]
    
    if isinstance(mm_points, np.ndarray):
        mm_points = common_utils.rotate_points_along_z(mm_points[np.newaxis, :, :], np.array([noise_rotation]))[0]
    ...
    return gt_boxes, points, bm_points, mm_points, noise_rotation


def global_scaling(gt_boxes, points, bm_points, mm_points, scale_range):
    """
    Args:
    ...
    if scale_range[1] - scale_range[0] < 1e-3:
        return gt_boxes, points, bm_points
    noise_scale = np.random.uniform(scale_range[0], scale_range[1])
    points[:, :3] *= noise_scale
    bm_points[:, :3] *= noise_scale
    gt_boxes[:, :6] *= noise_scale
    if isinstance(mm_points, np.ndarray):
        mm_points[:, :3] *= noise_scale
    
    return gt_boxes, points, bm_points, mm_points, noise_scale
...
def local_scaling(gt_boxes, points, bm_points, scale_range):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading]
        points: (M, 3 + C),
    ...
    for idx, box in enumerate(gt_boxes):
        ...
        points_in_box, mask = get_points_in_box(bm_points, box)
        
        # tranlation to axis center
        bm_points[mask, 0] -= box[0]
        bm_points[mask, 1] -= box[1]
        bm_points[mask, 2] -= box[2]
        
        # apply scaling
        bm_points[mask, :3] *= noise_scale
    ...
    noise_list = np.array(noise_list, dtype=np.float32)
    
    return gt_boxes, points, bm_points, noise_list

def local_rotation(gt_boxes, points, bm_points, rot_range):
    """
    Args:
    ...
    for idx, box in enumerate(gt_boxes):
        ...
        points_in_box, mask = get_points_in_box(bm_points, box)
        bm_points[mask, 0] -= centroid_x
        bm_points[mask, 1] -= centroid_y
        bm_points[mask, 2] -= centroid_z
        bm_points[mask, :] = common_utils.rotate_points_along_z(bm_points[np.newaxis, mask, :], np.array([noise_rotation]))[0]
        bm_points[mask, 0] += centroid_x
        bm_points[mask, 1] += centroid_y
        bm_points[mask, 2] += centroid_z
        ...
        noise_list.append(noise_rotation)
...
def noise_per_object_v3_(gt_boxes,
    ...
    if (points is not None) and (bm_points is not None):
        point_masks = box_np_ops.points_in_convex_polygon_3d_jit(
            points[:, :3], surfaces)
        bm_points_masks = box_np_ops.points_in_convex_polygon_3d_jit(
            bm_points[:, :3], surfaces)
        
        points_transform_(points, gt_boxes[:, :3], point_masks, loc_transforms,
                          rot_transforms, valid_mask)
        points_transform_(bm_points, gt_boxes[:, :3], bm_points_masks, loc_transforms,
                          rot_transforms, valid_mask)

        box3d_transform_(gt_boxes, loc_transforms, rot_transforms, valid_mask)

        point_masks = (point_masks.sum(axis=1) > 0)
        bg_points = points[~point_masks]
        fg_points = points[point_masks]

        large_sampled_gt_boxes = box_utils.enlarge_box3d(
            gt_boxes[:, 0:7], extra_width=data_aug_with_context
        )

        bg_points = box_utils.remove_points_in_boxes3d(bg_points, large_sampled_gt_boxes)
        points = np.concatenate([bg_points, fg_points])

    return points, loc_transforms, rot_transforms
