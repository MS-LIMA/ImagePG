...
class DataBaseSamplerMBM(object):
    ...
    def copy_paste_to_image(self, data_dict, gt_inmodal_segs, gt_number, point_idxes=None, shift_2d_list=None, sample_image_list=None):
        ...
        points_2d = points_2d.astype(np.int32)#floor

        inmodal_seg_list = [None] * (len(paste_order)-gt_number)
        # import pdb;pdb.set_trace()
        for _order in paste_order:
            # _order < gt_number
            single_seg_mask = gt_inmodal_segs[_order]
            if single_seg_mask is None:
                continue
            inmodal_ann_mask = maskUtils.decode(single_seg_mask) #(375, 1242)
            # print("### single_seg_mask.shape=", inmodal_ann_mask.shape)
            inmodal_ann_mask_coords = inmodal_ann_mask.nonzero()#y,x
            if _order >= gt_number:
                shifted_coords = (inmodal_ann_mask_coords[0]+shift_2d_list[_order-gt_number][1], inmodal_ann_mask_coords[1]+shift_2d_list[_order-gt_number][0])
                valid_mask = (shifted_coords[0]>=0) & (shifted_coords[0]<image.shape[0]) & (shifted_coords[1]>=0) & (shifted_coords[1]<image.shape[1])
                valid_shifted_coords = (shifted_coords[0][valid_mask], shifted_coords[1][valid_mask])
                valid_original_coords = (inmodal_ann_mask_coords[0][valid_mask], inmodal_ann_mask_coords[1][valid_mask])

                sample_image = sample_image_list[_order-gt_number]
                image[valid_shifted_coords] = sample_image[valid_original_coords] / 255
                
                # inmodal_ann_mask
                new_inmodal_ann_mask = np.zeros([image.shape[0], image.shape[1]])
                new_inmodal_ann_mask[valid_shifted_coords] = inmodal_ann_mask[valid_original_coords]
                # print("### new_inmodal_ann_mask.shape", new_inmodal_ann_mask.shape)
                inmodal_seg = maskUtils.encode(np.asfortranarray(new_inmodal_ann_mask.astype(np.uint8)))
                inmodal_seg_list[_order-gt_number] = inmodal_seg
                # import pdb;pdb.set_trace()
                # print('##inmodal_seg=', inmodal_seg);some = maskUtils.decode(inmodal_seg);print("## after decode", some.shape, 'depth=',boxes3d[_order, 0])


            else:
                # import pdb;pdb.set_trace()
                # print("### mark1", raw_image[inmodal_ann_mask_coords].mean())
                image[inmodal_ann_mask_coords] = raw_image[inmodal_ann_mask_coords]
                valid_shifted_coords = inmodal_ann_mask_coords
            ...
            paste_mask[valid_shifted_coords] = _order
            ...
            if _order < gt_number:
                fg_mask[valid_shifted_coords] = 1
        
        data_dict['segmentation'] = data_dict['segmentation'] + inmodal_seg_list
        data_dict['images'] = image

        if not self.joint_sample:
            return data_dict
        
        new_mask = (paste_mask[points_2d[:,1], points_2d[:,0]]==(point_idxes+gt_number)) & (point_idxes>=0)
        # import pdb;pdb.set_trace()
        if self.keep_raw:
            ...
            raw_mask = raw_mask & (point_idxes==-1)
        ...
        if 'annotation' in self.aug_use_type:
            ...
            # data_dict['num_original_point'] = keep_mask[:data_dict['num_original_point']].sum()
            # data_dict['points_2d'] = data_dict['points_2d'][keep_mask]
            data_dict['points'] = np.concatenate([data_dict['points'], keep_mask.reshape(-1, 1)*1], axis=1)
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, mv_height, sampled_gt_boxes2d, total_valid_sampled_dict):
        # data_dict and total_valid_sampled_dict is original
        gt_boxes_mask = data_dict['gt_boxes_mask']
        gt_boxes = data_dict['gt_boxes'][gt_boxes_mask]
        gt_names = data_dict['gt_names'][gt_boxes_mask]
        gt_inmodal_segs = [data_dict['segmentation'][i] for i in range(len(gt_boxes_mask)) if gt_boxes_mask[i]]
        data_dict['segmentation'] = copy.deepcopy(gt_inmodal_segs)
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            ori_points_2d, ori_depth_2d = sampled_calib.lidar_to_img(obj_points[:,:3])
            ...
            if self.point_refine:
                new_points_2d, new_depth_2d = data_dict['calib'].lidar_to_img(obj_points[:,:3])
                avg_shift_2d = (new_points_2d - ori_points_2d).mean(0)
                avg_shift_2d = (int(np.round(avg_shift_2d[0])), int(np.round(avg_shift_2d[1])))
                shift_2d_list.append(avg_shift_2d)

            gt_inmodal_segs.append(info['inmodal_seg'])
            ...
            if self.aug_with_img:
                ...
                sample_image_list.append(raw_image)
        ...
        # need further process at mask_points_and_boxes_outside_range and sample_points
        data_dict['num_original_point'] = len(points)
        ...
        ori_gt_idx = []
        for idx in range(len(data_dict['gt_names'])):
            if data_dict['gt_names'][idx] in self.class_names:
                ori_gt_idx.append(idx)
        boxes_gt_idx = np.array(ori_gt_idx).reshape(-1, 1).astype(np.int32)
        sampled_boxes_gt_idx = np.stack([x['gt_idx']  for x in total_valid_sampled_dict], axis=0).reshape(-1, 1).astype(np.int32)
        aug_boxes_gt_idx = np.concatenate([boxes_gt_idx, sampled_boxes_gt_idx], axis = 0)

        gt_idx = np.repeat(int(data_dict['frame_id']), len(boxes_gt_idx), axis=0).reshape(-1, 1).astype(np.int32)
        sample_gt_idx = np.stack([x['image_idx']  for x in total_valid_sampled_dict], axis=0).reshape(-1, 1).astype(np.int32)
        aug_boxes_image_idx = np.concatenate([gt_idx, sample_gt_idx], axis = 0)

        data_dict['aug_boxes_image_idx'] = aug_boxes_image_idx
        data_dict['aug_boxes_gt_idx'] = aug_boxes_gt_idx
        ...
        if self.aug_with_img:
            ...
            # print('shift_2d_list=', shift_2d_list)
            data_dict = self.copy_paste_to_image(data_dict, gt_inmodal_segs, gt_number, point_idxes, shift_2d_list, sample_image_list)
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                # TODO: save sampled_boxes before put_boxes_on_road_planes
                ...
                if self.aug_with_img:
                    ...
                    existed_boxes2d = data_dict['gt_boxes2d']
                    # print("sampled_boxes2d = ", sampled_boxes2d, 'existed_boxes2d', existed_boxes2d)
                    iou2d1 = box_utils.boxes_iou_normal(sampled_boxes2d, existed_boxes2d)
                    iou2d2 = box_utils.boxes_iou_normal(sampled_boxes2d, sampled_boxes2d)
                    ...
                    sampled_boxes2d = sampled_boxes2d[valid_mask]
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            # add visible mask for points
            vis_mask = np.ones([len(data_dict['points']), 1])
            data_dict['points'] = np.concatenate([data_dict['points'], vis_mask], axis=1)
