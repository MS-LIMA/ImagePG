...
class AddBestMatch(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.bmatch_infos = {}
        self.mlt_bm_root = {
            "Car":self.root_path.resolve() / sampler_cfg.PCN.CAR_MLT_BM_ROOT,
            "Cyclist":self.root_path.resolve() / sampler_cfg.PCN.CYC_MLT_BM_ROOT,
            "Pedestrian":self.root_path.resolve() / sampler_cfg.PCN.PED_MLT_BM_ROOT,
        }

        self.vis = False
        self.load_point_features = sampler_cfg.get("LOAD_POINT_FEATURES", 3)
        if sampler_cfg.get("ABLATION", None) is not None:
            self.rmv_self_occ = sampler_cfg.ABLATION.get("RMV_SELF_OCC", False)
            self.rmv_miss = sampler_cfg.ABLATION.get("RMV_MISS", False)
            self.num_point_features = sampler_cfg.ABLATION.get("NUM_POINT_FEATURES", 4)
            self.vis = sampler_cfg.ABLATION.get("VIS", False)
        ...
        for idx in range(gt_boxes_num):
            ...
            if gt_name in self.class_names:
                if 'aug_boxes_image_idx' in data_dict:
                    gt_img_id = data_dict['aug_boxes_image_idx'][idx]
                    gt_box_id = data_dict['aug_boxes_gt_idx'][idx]
                    file_path = self.mlt_bm_root[gt_name] / "{}_{}.pkl".format(gt_img_id.item(), gt_box_id.item())
    ...
    def combine_lst(self, bm_points_lst):
        if len(bm_points_lst) > 1:
            return np.concatenate(bm_points_lst, axis=0).reshape(-1, 3)
        elif len(bm_points_lst) == 1:
            return bm_points_lst[0].reshape(-1, 3)
        else:
    ...
    def __call__(self, data_dict):
        ...
        if self.sampler_cfg.get("ABLATION", None) is not None and len(data_dict['bm_points']) > 0:
            data_dict = self.filter_bm(data_dict)

        bm_points = self.combine_lst(data_dict['bm_points'])
