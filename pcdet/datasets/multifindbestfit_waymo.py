...
ann_file = 'waymo_processed_data_v0_5_0_waymo_dbinfos_train_sampled_1_parallel.pkl'
NUM_POINT_FEATURES = 5
N_CORES = max(16, multiprocessing.cpu_count())

start_index = 0
end_index = 9999999999

def extract_allpnts_mp(data_list, root_path):
    all_db_infos_lst = []
    box_dims_lst = []
    pnts_lst = []
    mirrored_pnts_lst = []
    for k in tqdm(range(len(data_list)), total=len(data_list), desc='loading'):
        info = data_list[k]
        obj_type = info['name']
        ...
        # obj_pnt_fpath = Path(root_path) / info['lidar_path']
        obj_pnt_fpath = Path(root_path) / 'train_mirror_target_temp' / (info['obj_ids'] + '.bin')
        if not os.path.exists(obj_pnt_fpath):
            continue
...
def extract_allpnts(root_path=None, splits=['train','val'], type='Car', apply_mirror=True):
    ...
    obj_ids_set = set()
    for split in splits:
        db_info_save_path = Path(root_path) / ann_file
        with open(db_info_save_path, 'rb') as f:
            ...
            for k in range(len(all_db_infos)):
                info = all_db_infos[k]
                if info['obj_ids'] in obj_ids_set:
                    continue
                obj_ids_set.add(info['obj_ids'])
                temp_list.append(info)
            temp_list = common_utils.split_list(temp_list, N_CORES)
            results = parmap.map(extract_allpnts_mp, temp_list, root_path, pm_pbar=True, pm_processes=N_CORES)
            for result in results:
                _all_db_infos_lst = result[0]
                _box_dims_lst = result[1]
                _pnts_lst = result[2]
                _mirrored_pnts_lst = result[3]

                all_db_infos_lst.extend(_all_db_infos_lst)
                box_dims_lst.extend(_box_dims_lst)
                pnts_lst.extend(_pnts_lst)
                mirrored_pnts_lst.extend(_mirrored_pnts_lst)
    return all_db_infos_lst, box_dims_lst, pnts_lst, mirrored_pnts_lst
...
def get_normalized_cloud(obj_pnt_fpath, gt_box, bottom=0.0):
    pnts = np.fromfile(str(obj_pnt_fpath), dtype=np.float32).reshape([-1,NUM_POINT_FEATURES])
...
def find_best_match_boxpnts(all_db_infos_lst, box_dims_lst, sorted_iou, pnt_thresh_best_iou_indices, mirrored_pnts_lst, pnts_lst, coords_num, occ_map, bm_dir, allrange, nx, ny, voxel_size, max_num_bm=5, num_extra_coords=2000, iou_thresh=0.84, ex_coords_ratio=10., nearest_dist=0.16, vis=False, save=False):
    ...
    '''
    from tqdm import tqdm
    for car_id in tqdm(range(0, len(mirrored_pnts_lst)), total=len(mirrored_pnts_lst)):
        ...
        if save:
            if bm_pnts.shape[0] > 4096:
                cur_pt_idxs = np.random.choice(bm_pnts.shape[0], 4096, replace=False)
                bm_pnts = bm_pnts[cur_pt_idxs]
            bm_pnts.astype(np.float32).tofile(os.path.join(bm_dir, '{}.bin'.format(info['obj_ids'])))
            # with open(os.path.join(bm_dir, image_idx+"_"+gt_idx+".pkl"), 'wb') as f:
            #     pickle.dump(bm_pnts.astype(np.float32), f)
            print("{}/{}: bm_vox_num {}, bm_pnt_num {} ".format(car_id, len(mirrored_pnts_lst), bm_coords_num, bm_pnts.shape))
...
if __name__ == '__main__':
    ...
    obj_types = ['Vehicle', 'Cyclist', 'Pedestrian']
    ...
    for i, obj_type in enumerate(obj_types):
        ...
        path = ROOT_DIR / 'data' / 'waymo'
        bm_dir_save_path = path / 'train_mirror_target'
