...
class DepthLSSTransform(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        lidar2cam = batch_dict['trans_lidar_to_cam']
        cam2img = batch_dict['trans_cam_to_img']
        camera_intrinsics = batch_dict['camera_intrinsics']

        cam2lidar = np.linalg.inv(lidar2cam)
        lidar2image = (cam2img @ lidar2cam)
        ...
        camera2lidar_rots = cam2lidar[..., :3, :3]
        camera2lidar_trans = cam2lidar[..., :3, 3]

        points = batch_dict['points']

        batch_size = batch_dict['batch_size']
        depth = torch.zeros(batch_size, 1, 1, *self.image_size).to(points[0].device)
