...
class HeightCompression(nn.Module):
    ...
    def bev_align(self, batch_dict, bev_feat, points_trans, rot_angle, flip_x, scale, stride=8):
        ...
        for b in range(batch_size):
            ...
            if flip_x:
                grid_pts_b[:, 1] *= -1
            grid_pts_b = common_utils.rotate_points_along_z(grid_pts_b.unsqueeze(0), angle=rot_angle).squeeze(0)
            if scale != 1.0:
                grid_pts_b *= scale
            ...
            # vis.add_points(points_trans[points_trans[..., 0] == b][..., 1:4], [1, 1, 0])
            # vis.add_points(grid_pts_b, [1, 1, 0])
            # vis.add_points(grid_pts.cpu().numpy())
            # vis.show()
    ...
    def forward(self, batch_dict):
        ...
        if 'rtspconv' in batch_dict:
            encoded_spconv_tensor_rot = batch_dict['encoded_spconv_tensor_rot']

            trans_dict = batch_dict['trans_dict']
            rotation_angles = trans_dict['rotation_angles']
            flip_x = trans_dict['flip_x']
            scales = trans_dict['scales']
            points_trans = batch_dict['points_trans']

            spatial_features_list = [spatial_features]
            batch_size = batch_dict['batch_size']

            for tensor, rot_angle, f_x, scale, p_t in zip(encoded_spconv_tensor_rot, rotation_angles, flip_x, scales, points_trans):
                spatial_features = self.extract_features(tensor['encoded_spconv_tensor'])
                spatial_features = self.bev_align(batch_dict, spatial_features, p_t, rot_angle, f_x, scale, batch_dict['encoded_spconv_tensor_stride'])
