...
class CenterHeadIOU(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.iou_weight = self.model_cfg.IOU_WEIGHT
    ...
    def _iou_target(self, example, preds_dict, task_id, out_size_factor):
        ...
        batch_det_reg = preds_dict['center']
        batch_det_hei = preds_dict['center_z']
        ...
        iou_targets = iou3d_nms_utils.boxes_iou3d_gpu(batch_box_preds.reshape(-1, 7), batch_box_targets.reshape(-1, 7))[range(
    ...
    def assign_targets(self, gt_boxes, feature_map_size=None, **kwargs):
        ...
        if 'vel' not in self.separate_head_cfg.HEAD_DICT.keys():
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            final_pred_dicts = centernet_utils.decode_bbox_from_heatmap(
                ...
                batch_iou=batch_iou, point_cloud_range=self.point_cloud_range, voxel_size=self.voxel_size,
    ...
    def get_box_center(self, boxes):
        ...
        for box in boxes:            
            if self.num_point == 1 or len(box['pred_boxes']) == 0:
                ...
                front_middle = torch.cat([(corners[:, 0, :2] + corners[:, 1, :2])/2, height], dim=-1)
    ...
    def absl_to_relative(self, absolute):
        a1 = (absolute[..., 0] - self.point_cloud_range[0]) / self.voxel_size[0] / self.feature_map_stride 
        a2 = (absolute[..., 1] - self.point_cloud_range[1]) / self.voxel_size[1] / self.feature_map_stride 
    ...
    def forward(self, data_dict):
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            if self.predict_boxes_when_training or not self.training:
                features = []
                bev_features = data_dict['spatial_features_2d'].permute(0, 2, 3, 1).contiguous()
                centers_frame = self.get_box_center(pred_dicts)
                batch_size = data_dict['batch_size']
                ret_maps = [] 
                for batch_idx in range(batch_size):
                    xs, ys = self.absl_to_relative(centers_frame[batch_idx])
                    # N x C 
                ...
                features.append(ret_maps)
                rois, roi_scores, roi_labels, roi_features = self.reorder_rois_for_refining_features(data_dict['batch_size'], pred_dicts, features)
                data_dict['rois'] = rois
                data_dict['roi_scores'] = roi_scores
                data_dict['roi_labels'] = roi_labels
                data_dict['roi_features'] = roi_features
                data_dict['has_class_labels'] = True

                if not self.training:
                    data_dict['rebuilt_points'] = None
                    data_dict['batch_box_preds'] = rois
                    data_dict['batch_cls_preds'] = roi_scores.unsqueeze(-1)
