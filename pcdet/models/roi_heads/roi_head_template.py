...
from ...utils import box_coder_utils, common_utils, loss_utils, box_utils, bbloss
...
class RoIHeadTemplate(nn.Module):
    def __init__(self, num_class, model_cfg, **kwargs):
        ...
        for i in range(self.model_cfg.get('REFINE_STAGES', 1)):
    ...
    def assign_targets(self, batch_dict, stage=0):
        
        batch_size = batch_dict['batch_size']
        
        with torch.no_grad():
            targets_dict = self.proposal_target_layers[stage].forward(batch_dict)
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        total_loss_reg = torch.zeros(1).cuda()
        total_loss_corner = torch.zeros(1).cuda()
        ...
        for forward_ret_dict in forward_ret_dict_list:
            ...
            if loss_cfgs.REG_LOSS == 'smooth-l1':
                ...
                if len(gt_boxes3d_ct[fg_mask]) == 0:
                    ...
                    b_loss = bbloss.bb_loss(batch_box_preds[fg_mask], gt_boxes3d_ct[fg_mask]).sum()
                    b_loss = b_loss / (fg_mask.sum() + 1)

                rcnn_loss_reg = rcnn_loss_reg + b_loss
                total_loss_reg = total_loss_reg + rcnn_loss_reg

            else:
                raise NotImplementedError
            
        #total_loss_reg = total_loss_reg / len(forward_ret_dict_list)
        #total_loss_corner = total_loss_corner / len(forward_ret_dict_list)
    
        total_loss = total_loss_reg + total_loss_corner
        total_loss = total_loss.view([])
        
        return total_loss, {'rcnn_loss_reg' : total_loss_reg.item(), 'rcnn_loss_corner' : total_loss_corner.item()}
    ...
    def get_box_cls_layer_loss(self, forward_ret_dict):
        ...
        for forward_ret_dict in forward_ret_dict_list:
            ...
            total_loss += rcnn_loss_cls
            tb_dict = {'rcnn_loss_cls': rcnn_loss_cls.item()}
        
        # total_loss /= len(forward_ret_dict_list)
        
        return total_loss, {'rcnn_loss_cls' : total_loss.item()}
