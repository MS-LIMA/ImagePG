...
class GeneralizedLSSFPN(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.freeze = self.model_cfg.get("FREEZE", False)
        ...
        self.channel_reduction = self.model_cfg.get('CHANNEL_REDUCTION', False)
        if self.channel_reduction:
            self.conv_reduce = nn.Sequential(
                nn.Conv2d(out_channels, self.model_cfg.OUT_CHANNELS_RED,
                          kernel_size=1, padding=1, bias=False),
                nn.BatchNorm2d(self.model_cfg.OUT_CHANNELS_RED),
    ...
    def forward(self, batch_dict):
        ...
        if self.channel_reduction:
            out = self.conv_reduce(batch_dict['image_fpn'][0])
            if batch_dict['batch_size'] != out.shape[0]:
                out = out.view(batch_dict['batch_size'], out.shape[0] // batch_dict['batch_size'], out.shape[-3], out.shape[-2], out.shape[-1])
