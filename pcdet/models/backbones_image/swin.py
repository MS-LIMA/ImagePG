...
class SwinTransformer(nn.Module):
    ...
    def create_depth_image(self, batch_dict):
        ...
        for i in range(B):
            ...
            pts /= noise_scale
            pts = common_utils.rotate_points_along_z(pts.unsqueeze(0), -noise_rotation).squeeze(0)
            
            if flip_x:
                pts[:,1] *= -1
                proj = torch.tensor(calib.lidar_to_img(pts[...,:3].cpu())[0], device=points.device)
                proj[:,0] = W - proj[:, 0]
            else:
                proj = torch.tensor(calib.lidar_to_img(pts[...,:3].cpu())[0], device=points.device)

            pad_size = proj.new_tensor(batch_dict['pad_size'][i]) / batch_dict['img_scale'][i]
            if flip_x:
                proj[:,1] = proj[:,1] - pad_size[0] # H
                proj[:,0] = proj[:,0] - pad_size[1] # W
                
            mask = proj[:,1] >=0 & proj[:,1] < H & proj[:,0] >= 0 & proj[:,0] < W
            proj = proj[mask]
            
            depth_img = pts.new_zeros((1, H, W))
            depth_img[0, proj[:,1], proj[:,0]] = proj[:,2]
            depth_img = torchvision.transforms.Resize((img.shape[1], img.shape[2]), 
                                                      interpolation=torchvision.transforms.InterpolationMode.NEAREST, 
                                                      antialias=False)(depth_img)
    ...
    def forward(self, batch_dict):
        ...
        #        outs[i] = outs[i].view(B // D, D, outs[i].shape[-3], outs[i].shape[-2], outs[i].shape[-1])
