...
#         encoder_layers = nn.TransformerEncoderLayer(attention_cfg.NUM_FEATURES, attention_cfg.NUM_HEADS, attention_cfg.NUM_HIDDEN_FEATURES, attention_cfg.DROP_PATH_RATE)
...
def get_positional_encoder(attention_cfg):
    ...
    if attention_cfg.POSITIONAL_ENCODER == 'frequency':
        ...
        pos_encoder = FeedForwardPositionalEncoding(d_input=1, d_output=attention_cfg.ENCODER.NUM_FEATURES)
        input_feats = 1
    elif attention_cfg.POSITIONAL_ENCODER == 'density_grid_points':
        pos_encoder = FeedForwardPositionalEncoding(d_input=4, d_output=attention_cfg.ENCODER.NUM_FEATURES)
        input_feats = 4
    elif attention_cfg.POSITIONAL_ENCODER == 'density_grid_points_corners':
        pos_encoder = FeedForwardPositionalEncoding(d_input=31, d_output=attention_cfg.ENCODER.NUM_FEATURES)
        input_feats = 31
    elif attention_cfg.POSITIONAL_ENCODER == 'density_centroid':
        pos_encoder = FeedForwardPositionalEncoding(d_input=7, d_output=attention_cfg.ENCODER.NUM_FEATURES)
        input_feats = 7
    
    return pos_encoder, input_feats