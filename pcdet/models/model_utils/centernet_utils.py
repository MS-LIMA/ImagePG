...
def sy_draw_gaussian_to_heatmap(heatmap, center, radius, class_idx):
    ...
    for i in range(radius):
        for j in range(radius):
            ...
            if not (0 <= i < heatmap.shape[0] and 0 <= j < heatmap.shape[1]):
                continue
            
            c_center_gaussian = center + torch.tensor([i, j]).cuda()
            c_center_gaussian[:, 0] = torch.clamp(c_center_gaussian[:, 0], min=0, max=199)
            c_center_gaussian[:, 1] = torch.clamp(c_center_gaussian[:, 1], min=0, max=175)
...
def _topk(scores, batch_iou=None, K=40, vel=None):
    batch, num_class, height, width = scores.size()
    scores = scores.flatten(2, 3)
    if batch_iou is not None:
        batch_iou = torch.clamp(batch_iou.reshape(batch, 1, height*width), min=0, max=1)
        if vel is None:
            scores = scores * torch.pow(batch_iou, 2)
...
def decode_bbox_from_heatmap(heatmap, rot_cos, rot_sin, center, center_z, dim,
                             point_cloud_range=None, voxel_size=None, feature_map_stride=None, vel=None, batch_iou = None, K=100,
    ...
    scores, inds, class_ids, ys, xs = _topk(heatmap, batch_iou=batch_iou, K=K, vel=vel)
...
def decode_bbox_from_heatmap_org(heatmap, rot_cos, rot_sin, center, center_z, dim,
