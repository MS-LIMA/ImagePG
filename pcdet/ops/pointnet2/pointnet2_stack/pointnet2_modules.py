...
def build_local_aggregation_module(input_channels, config):
    ...
    elif local_aggregation_name == 'Acc':
        mlps = config.MLPS
        for k in range(len(mlps)):
            mlps[k] = [input_channels] + mlps[k]
        cur_layer = AcceleratedStackSAModuleMSG(
            radii=config.POOL_RADIUS, nsamples=config.NSAMPLE, mlps=mlps, use_xyz=True, pool_method='max_pool',
        )
        num_c_out = sum([x[-1] for x in mlps])
...
class AcceleratedStackSAModuleMSG(nn.Module):
    ...
    def __init__(self, *, radii: List[float], nsamples: List[int], mlps: List[List[int]],
        ...
        for i in range(len(radii)):
            ...
            cur_mlp_pos = nn.Sequential(
                nn.Conv2d(3, mlp_spec[1] + 3, kernel_size=1, bias=False),
                nn.BatchNorm2d(mlp_spec[1] + 3)
            )

            cur_mlp_out = nn.Sequential(
                nn.Conv1d(mlp_spec[1] + 3, mlp_spec[2], kernel_size=1, bias=False),
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features=None, empty_voxel_set_zeros=True):
        ...
        for k in range(len(self.groupers)):
            ...
            grouped_features, ball_idxs = self.groupers[k](
                xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features_in
            )  # (M1 + M2, C, nsample)
            grouped_xyz, grouped_features = grouped_features[:, 0:3, :], grouped_features
            grouped_features = grouped_features.permute(1, 0, 2).unsqueeze(dim=0)  # (1, C, M1 + M2 ..., nsample)
