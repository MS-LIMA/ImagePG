...
class NeighborVoxelSAModuleMSG(nn.Module):
    ...
    def precompute(self, batch_dict, src_name, features_list):
        lengths = [t.size(0) for t in features_list]
        idx_tensor = torch.arange(len(lengths), device=features_list[0].device) \
                        .repeat_interleave(torch.tensor(lengths, device=features_list[0].device))
        # output: (sum(lengths), 4) 형태
        output = torch.cat([
            idx_tensor.unsqueeze(1),            # (sum,1)
            torch.cat(features_list, dim=0)     # (sum,3)
        ], dim=1)
        
        for k in range(len(self.groupers)):
            # features_in: (1, C, M1+M2)
            features_in = output[..., 1:].permute(1, 0).unsqueeze(0)
            ...
            batch_dict[src_name] = torch.cat([output[..., 0:1], features_in], dim=1)
        
    def forward_fast(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, \
        ...
        for k in range(len(self.groupers)):
            ...
            # empty_balls_list.append(empty_ball_mask.view(-1, 1))
        
        # (M1 + M2 ..., C)
        new_features = torch.cat(new_features_list, dim=1)
        # empty_balls = torch.cat(empty_balls_list, dim=1)
        return new_features
