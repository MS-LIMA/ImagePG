...
def ciou_loss(pred_boxes, target_boxes):
        """
        Complete IoU loss 계산 함수
        pred_boxes와 target_boxes는 [x1, y1, x2, y2] 형식의 텐서입니다.
        """
        # 예측 박스 좌표
...
def remove_points_not_in_boxes3d(points, boxes3d):

    boxes3d, is_numpy = common_utils.check_numpy_to_torch(boxes3d)
    points, is_numpy = common_utils.check_numpy_to_torch(points)
    point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(points[:, 0:3], boxes3d[:, 0:7])
...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    boxes_b, is_numpy_1 = common_utils.check_numpy_to_torch(boxes_b)
    assert is_numpy==is_numpy_1
    assert boxes_a.shape[1] == boxes_b.shape[1] == 4
    x_min = torch.max(boxes_a[:, 0, None], boxes_b[None, :, 0])
    x_max = torch.min(boxes_a[:, 2, None], boxes_b[None, :, 2])
    y_min = torch.max(boxes_a[:, 1, None], boxes_b[None, :, 1])
    ...
    a_intersect_b = x_len * y_len
    iou = a_intersect_b / torch.clamp_min(area_a[:, None] + area_b[None, :] - a_intersect_b, min=1e-6)
    return iou.numpy() if is_numpy else iou
