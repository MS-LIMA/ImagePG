...
class Visualizer3D():
    def __init__(self, point_size=3, background_color = [0, 0, 0]):
        vis = open3d.visualization.Visualizer()
        vis.create_window()

        vis.get_render_option().point_size = point_size
        vis.get_render_option().background_color = np.array(background_color, dtype=np.float32)
        self.vis = vis
    ...
    def draw_scenes(self, points=None, keypoints=None, gt_boxes=None, roi_boxes=None, ref_boxes=None, ref_labels=None, ref_scores=None, point_colors=None, draw_origin=True, kp_color=0):
    ...
    #        points = points[cur_target_points_in_roi.sum(axis=0)==1]
        ...
        if roi_boxes is not None:
            vis = self.draw_box(vis, roi_boxes, (1, 0, 0))

    def add_points(self, points, point_colors=None):
        if isinstance(points, torch.Tensor):
    ...
    def draw_box(self, gt_boxes, color=[0, 1, 0], ref_labels=None, score=None):
        ...
        if isinstance(gt_boxes, torch.Tensor):
            gt_boxes = gt_boxes.cpu().detach()
            
        vis = self.vis
        for i in range(gt_boxes.shape[0]):
            line_set, box3d = self.translate_boxes_to_open3d_instance(gt_boxes[i])
            if ref_labels is None:
                line_set.paint_uniform_color(np.array(color, dtype=np.float32))
            else:
